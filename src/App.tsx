...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      const storedFunds = getStoredFunds();
      if (storedFunds.length > 0) {
        Promise.all(storedFunds.map(async (item) => {
          const data = await fetchFundData(item.code);
          return {
            ...data,
            share: item.share,
  ...
  const handleAddFund = async (item: FundStorageItem) => {
    ...
    if (newData.error) {
      message.warning(`基金 ${item.code} 数据获取失败: ${newData.error}，但已添加到列表`);
    }
  ...
  return (
    <Layout className="layout" style={{ minHeight: '100vh' }}>
      <Header style={{ display: 'flex', alignItems: 'center' }}>
        <FundOutlined style={{ color: 'white', fontSize: '24px', marginRight: '10px' }} />
        <Title level={3} style={{ color: 'white', margin: 0 }}>实时基金估值助手</Title>
      </Header>
      ...
      <Footer style={{ textAlign: 'center' }}>
        Real-time Fund Valuation ©{new Date().getFullYear()} Created with React & Ant Design
      </Footer>
    </Layout>
  );
};
